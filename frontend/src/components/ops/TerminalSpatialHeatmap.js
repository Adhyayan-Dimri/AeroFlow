...
export default function TerminalSpatialHeatmap() {
  ...
  const zonesData = useMemo(() => {
    return TERMINAL_ZONES.map((zone) => {
      ...
      const color = getDensityColor(ratio, isDark);
  ...
  }, [liveMetrics, hourMultiplier, isDark]);
  ...
  return (
    <div className="p-5 sm:p-6 space-y-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm dark:shadow-xl" data-testid="terminal-spatial-heatmap">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          ...
          <h2 className="font-display text-xl sm:text-2xl font-bold mt-1 text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <span className="text-xs font-mono font-bold px-2.5 py-0.5 rounded-md bg-cyan-100 dark:bg-cyan-500/10 text-cyan-800 dark:text-cyan-400 border border-cyan-300 dark:border-cyan-500/30">
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900/90 p-2 rounded-xl border border-slate-200 dark:border-slate-800 shrink-0 shadow-sm">
          <button
            onClick={() => setIsPlaying(!isPlaying)}
            title={isPlaying ? "Pause replay" : "Play rush simulation"}
            className="w-8 h-8 rounded-lg bg-cyan-100 dark:bg-cyan-500/20 hover:bg-cyan-200 dark:hover:bg-cyan-500/30 text-cyan-700 dark:text-cyan-300 grid place-items-center transition-colors border border-cyan-300 dark:border-cyan-500/40"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        ...
        <div className="xl:col-span-8 rounded-2xl bg-white dark:bg-[#070E14] border border-slate-200 dark:border-slate-800 p-4 relative overflow-hidden shadow-sm dark:shadow-inner">
                ...
                return (
                  ...
                  >
                    ...
                    <rect
                      ...
                      stroke={isSelected ? (isDark ? "#FFFFFF" : "#0F172A") : zone.color.stroke}
          ...
          {/* Map Color Legend */}
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-200 dark:border-slate-800/80 text-[11px] font-mono text-slate-600 dark:text-slate-400 flex-wrap gap-2">
            <span className="font-bold text-slate-800 dark:text-slate-200">Density Scale:</span>
            <div className="flex items-center gap-3 flex-wrap">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-cyan-500/30 border border-cyan-500" />
                <span className="text-cyan-700 dark:text-cyan-300 font-bold">Lull (&lt;35%)</span>
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-emerald-500/30 border border-emerald-500" />
                <span className="text-emerald-700 dark:text-emerald-300 font-bold">Moderate (35–65%)</span>
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-amber-500/40 border border-amber-500" />
                <span className="text-amber-700 dark:text-amber-300 font-bold">Heavy (65–85%)</span>
              </span>
              <span className="flex items-center gap-1.5">
        ...
        <div className="xl:col-span-4 space-y-4">
          <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-md dark:shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="p-3 rounded-xl bg-white dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 shadow-sm">
                <div className="text-[10px] uppercase font-mono text-slate-500 dark:text-slate-400 font-bold">Current Occupancy</div>
              ...
              <div className="p-3 rounded-xl bg-white dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 shadow-sm">
                <div className="text-[10px] uppercase font-mono text-slate-500 dark:text-slate-400 font-bold">Estimated Wait</div>
            ...
            <div className="p-3.5 rounded-xl bg-cyan-50 dark:bg-cyan-950/20 border border-cyan-200 dark:border-cyan-500/30 space-y-1.5 shadow-sm">
              ...
              {[...zonesData]
                ...
                .map((z, idx) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2.5 rounded-xl bg-white dark:bg-slate-950/50 hover:bg-slate-100 dark:hover:bg-slate-800/70 border border-slate-200 dark:border-slate-800 transition-all text-left shadow-sm group"
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <ChevronRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-cyan-600 dark:group-hover:text-cyan-400" />
                    </div>
                  </button>
                ))}
            </div>
