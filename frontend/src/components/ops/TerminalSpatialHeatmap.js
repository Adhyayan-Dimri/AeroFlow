...
export default function TerminalSpatialHeatmap() {
  ...
  return (
    <div className="aero-card p-5 sm:p-6 space-y-6" data-testid="terminal-spatial-heatmap">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-aero-border pb-5">
        <div>
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-ping" />
            <span className="overline text-cyan-400 font-bold tracking-wider">
        ...
        <div className="flex items-center gap-3">
          <div className="px-3.5 py-2 rounded-xl bg-slate-900/80 border border-slate-700/60 flex items-center gap-2.5 shadow-sm">
            <Users className="w-4 h-4 text-cyan-400" />
            <div>
              <div className="text-[10px] text-slate-400 uppercase font-mono">Terminal Load</div>
              <div className="font-mono font-black text-sm text-cyan-400">
                {totalTerminalPax.toLocaleString()} <span className="text-[10px] font-normal text-slate-400">pax</span>
              </div>
            </div>
          </div>
          ...
          <div className="px-3.5 py-2 rounded-xl bg-slate-900/80 border border-slate-700/60 flex items-center gap-2.5 shadow-sm">
            ...
            <div>
              <div className="text-[10px] text-slate-400 uppercase font-mono">Peak Hotspot</div>
              <div className="font-mono font-black text-sm text-rose-400">
                {highestHotspot?.densityPct}% <span className="text-[10px] font-normal text-slate-400">cap</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-slate-900/90 p-2 rounded-xl border border-slate-800 shrink-0">
          <button
            onClick={() => setIsPlaying(!isPlaying)}
            title={isPlaying ? "Pause replay" : "Play rush simulation"}
            className="w-8 h-8 rounded-lg bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 grid place-items-center transition-colors border border-cyan-500/40"
          >
            {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 fill-cyan-300" />}
          </button>
          <div className="flex items-center gap-2">
            <Clock className="w-3.5 h-3.5 text-cyan-400" />
            <span className="font-mono font-black text-xs text-white min-w-[58px]">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        ...
        <div className="xl:col-span-8 rounded-2xl bg-[#070E14] border border-slate-800 p-4 relative overflow-hidden shadow-inner">
          <div className="flex items-center justify-between text-xs text-slate-400 font-mono mb-2">
            ...
            <span className="text-[11px] text-cyan-400">Click any zone for AI balancing actions</span>
          </div>
          ...
          <div className="relative w-full aspect-[640/680] max-h-[580px]">
            ...
            >
              ...
              <rect x="20" y="20" width="600" height="640" rx="12" fill="none" stroke="rgba(6, 182, 212, 0.2)" strokeWidth="1.5" strokeDasharray="6 4" />
          ...
          {/* Map Color Legend */}
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-800/80 text-[11px] font-mono text-slate-400 flex-wrap gap-2">
            <span className="font-bold text-slate-300">Density Scale:</span>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-3 rounded bg-rose-500/50 border border-rose-400 shadow-[0_0_6px_rgba(244,63,94,0.6)]" />
          ...
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-4">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400 font-mono mt-0.5">
                Operational Staffing Key: <span className="text-slate-200">{activeZoneDetail?.staffingKey}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800">
                <div className="text-[10px] uppercase font-mono text-slate-400">Current Occupancy</div>
                <div className="font-mono font-black text-lg text-white mt-0.5">
              ...
              <div className="p-3 rounded-xl bg-slate-950/70 border border-slate-800">
                <div className="text-[10px] uppercase font-mono text-slate-400">Estimated Wait</div>
            ...
            <div className="text-[11px] text-slate-400 flex items-center justify-between pt-1">
              ...
              <span className="font-mono text-white font-bold">{String(simHour).padStart(2, "0")}:00 IST</span>
            </div>
          </div>
              ...
              {[...zonesData]
                ...
                .map((z, idx) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-2 rounded-lg bg-slate-950/50 hover:bg-slate-800/70 border border-slate-800 transition-all text-left group"
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-4 h-4 rounded-full bg-slate-800 text-[10px] font-mono font-bold flex items-center justify-center text-slate-400">
                        {idx + 1}
                      </span>
                      <span className="text-xs font-bold text-slate-300 group-hover:text-cyan-300 truncate max-w-[150px]">
                        {z.name}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
