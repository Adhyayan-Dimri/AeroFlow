        ...
        <div className="lg:col-span-8 p-5 sm:p-7 space-y-6">
          ...
          <div className="flex items-center justify-between gap-4 py-2">
            ...
            <div className="space-y-1">
              <div className="overline text-aero-t3 text-[10px] font-bold">ORIGIN</div>
              <div className="font-display font-black text-3xl sm:text-4xl text-slate-900 dark:text-white tracking-tight">
                {flight.origin || "DEL"}
              </div>
              <div className="text-xs font-semibold text-aero-t2 flex items-center gap-1">
                <MapPin className="w-3 h-3 text-aero-cyan shrink-0" />
                {flight.origin === "DEL" ? "Delhi · IGI T3" : flight.origin}
              </div>
            </div>

            {}
            <div className="flex-1 flex flex-col items-center justify-center px-3 sm:px-6 relative">
              <div className="text-[11px] font-mono font-bold text-aero-t3 mb-1.5 flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-aero-cyan" />
              ...
              <div className="w-full flex items-center relative py-1">
                ...
                {}
                <div className={`absolute left-1/2 -translate-x-1/2 w-8 h-8 rounded-full ${theme.accentBg} border ${theme.accentBorder} grid place-items-center shadow-md`}>
                  <Plane className={`w-4 h-4 ${theme.accentText} ${isDep ? "rotate-45" : "rotate-[135deg]"}`} />
                </div>
              </div>

              <div className="text-[10px] font-mono text-aero-t2 mt-1.5 font-bold flex items-center gap-1.5">
                <Clock className="w-3 h-3 text-aero-cyan" />
                <span>Air Time: {durationFormatted}</span>
                <span className="text-aero-t3">•</span>
                <span className="text-aero-t3 uppercase">{isDep ? "Outbound" : "Inbound"}</span>
              </div>
            </div>

            {}
            <div className="space-y-1 text-right">
              <div className="overline text-aero-t3 text-[10px] font-bold">DESTINATION</div>
              <div className="font-display font-black text-3xl sm:text-4xl text-slate-900 dark:text-white tracking-tight">
                {flight.destination || "BOM"}
              </div>
              <div className="text-xs font-semibold text-aero-t2 flex items-center justify-end gap-1">
                {flight.destination === "DEL" ? "Delhi · IGI T3" : flight.destination}
                <MapPin className="w-3 h-3 text-aero-cyan shrink-0" />
              </div>
            </div>
          </div>
